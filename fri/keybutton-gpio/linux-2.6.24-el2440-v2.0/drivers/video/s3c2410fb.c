...
#define S3C2410_PULLUP_EN 0
#define S3C2410_PULLUP_DIS 1

#define S3C2410_DATA_LOW 0
#define S3C2410_DATA_HIGH 1

static unsigned long va_spi;
...
//	saddr2 += info->fix.line_length * info->var.yres;
	saddr2 += (info->var.xres * info->var.yres*info->var.bits_per_pixel)/8;
	saddr2 >>= 1;

	saddr3 = S3C2410_OFFSIZE(0) |
//		 S3C2410_PAGEWIDTH((info->fix.line_length / 2) & 0x3ff);
		 S3C2410_PAGEWIDTH(info->var.xres);

	printk("LCDSADDR1 = 0x%08lx\n", saddr1);
	printk("LCDSADDR2 = 0x%08lx\n", saddr2);
	printk("LCDSADDR3 = 0x%08lx\n", saddr3);
					  ...
					  unsigned long pixclk)
{
	...
	printk("pixclk %ld, divisor is %ld\n", pixclk, (long)div);
	return div;
}
			       ...
			       struct fb_info *info)
{
	...
	if (!display) {
		printk("wrong resolution or depth %dx%d at %d bpp\n",
					     ...
					     struct s3c2410fb_hw *regs)
{
	...
	/* update X/Y info */
	printk("setting horz: lft=%d, rt=%d, sync=%d\n",
					     ...
					     struct s3c2410fb_hw *regs)
{
	...
	/* update X/Y info */
	printk("setting vert: up=%d, low=%d, sync=%d\n",
...
static int s3c2410fb_blank(int blank_mode, struct fb_info *info)
{
	...
	void __iomem *regs = fbi->io;
   	...
   	blank_mode = FB_BLANK_UNBLANK; // added 
	...
	if (blank_mode == FB_BLANK_UNBLANK)
		...
		printk("setting TPAL to output 0x000000\n");
...
static int s3c2410fb_init_registers(struct fb_info *info)
{
	...
	s3c2410_gpio_pullup(S3C2410_GPG4, S3C2410_PULLUP_EN);
	s3c2410_gpio_cfgpin(S3C2410_GPG4, S3C2410_GPG4_OUTP);
	s3c2410_gpio_setpin(S3C2410_GPG4, S3C2410_DATA_HIGH);
	...
	s3c2410fb_set_lcdaddr(info); // added
...
#define cs_low()	s3c2410_gpio_setpin(S3C2410_GPG3, 0)
#define cs_high()	s3c2410_gpio_setpin(S3C2410_GPG3, 1)

#define S3C24XX_VA_SPI va_spi  // added 2.6.24
...
void spi_tx(unsigned char index, unsigned short data)
{
	...
	// setting index data
	tmp[0] = 0x74;  // device ID code(011101) RS(0) RW(0)
	tmp[1] = index;
	tmp[2] = 0x0;
	// setting instruction data
	tmp[3] = 0x76;
	tmp[4] = ((data & 0xff00)>>8) & 0xff;
	tmp[5] = (data & 0xff);

	cs_low();
	for(i=0;i<6;i++){
		if(i!=2)	writel(tmp[i], S3C2410_SPTDAT1);
		while(!(readl(S3C2410_SPSTA1)&1));
		if(i==2){
			cs_high();
			writel( (readl(S3C2410_SPCON1)& ~(1<<4)), S3C2410_SPCON1 );
			cs_low();
			writel( (readl(S3C2410_SPCON1)|(1<<4)), S3C2410_SPCON1 );
		}
	}
...
{
	struct clk *spi_clk;
	int gpgcon_bkup = readl(S3C2410_GPGCON);
	int gpgdat_bkup = readl(S3C2410_GPGDAT);
	int gpgup_bkup  = readl(S3C2410_GPGUP);

	va_spi = (unsigned long)ioremap(0x59000000,0x38);	// added 2.6.24

	// lcd init. using SPI1 channel
	printk("SPI LCD Init ... ");

	spi_clk = clk_get(NULL, "spi");
	if(IS_ERR(spi_clk))		panic("failed to get clock of SPI");
	clk_enable(spi_clk);
	//clk_use(spi_clk);
	clk_put(spi_clk);

	// SPI1 port initialization
	s3c2410_gpio_pullup(S3C2410_GPG6, 0);	// SPIMOSI1
	s3c2410_gpio_pullup(S3C2410_GPG7, 0);	// SPICLK1
	s3c2410_gpio_pullup(S3C2410_GPG3, 0);	// nSS1
	...
	writel( (0x2), S3C2410_SPPRE1 );
	writel( ((S3C2410_SPCON_SMOD_POLL)|(S3C2410_SPCON_ENSCK)|(S3C2410_SPCON_MSTR)|(S3C2410_SPCON_CPOL_HIGH)|(S3C2410_SPCON_CPHA_FMTB)|(S3C2410_SPCON_TAGD_NORM)), S3C2410_SPCON1 );
	writel( ((S3C2410_SPPIN_ENMUL_DIS)|(S3C2410_SPPIN_RESERVED)|(S3C2410_SPPIN_KEEP)), S3C2410_SPPIN1 );

	mdelay(50);

	// Init_Lcd_Function
	spi_tx(0x01,0x001d);    spi_tx(0x02,0x0000);    spi_tx(0x03,0x0000);
	spi_tx(0x04,0x0000);    spi_tx(0x05,0x50a3);    spi_tx(0x06,0x0000);
	spi_tx(0x07,0x0000);    spi_tx(0x08,0x0000);    spi_tx(0x09,0x0000);
	spi_tx(0x0a,0x0000);    spi_tx(0x10,0x0000);    spi_tx(0x11,0x0000);
	spi_tx(0x12,0x0000);    spi_tx(0x13,0x0000);    spi_tx(0x14,0x0000);
	spi_tx(0x15,0x0000);    spi_tx(0x16,0x0000);    spi_tx(0x17,0x0000);
	spi_tx(0x18,0x0000);    spi_tx(0x19,0x0000);
	mdelay(10);
	...
	//  Set_Disp_Function
	spi_tx(0x01,0x409d);    spi_tx(0x02,0x0204);    spi_tx(0x03,0x2100);
	spi_tx(0x04,0x1000);    spi_tx(0x05,0x5003);    spi_tx(0x06,0x0009);
	spi_tx(0x07,0x000f);    spi_tx(0x08,0x0800);    spi_tx(0x10,0x0000);
	spi_tx(0x11,0x0000);    spi_tx(0x12,0x000f);    spi_tx(0x13,0x1f00);
	spi_tx(0x14,0x0000);    spi_tx(0x15,0x0000);    spi_tx(0x16,0x0000);
	spi_tx(0x17,0x0000);    spi_tx(0x18,0x0000);    spi_tx(0x19,0x0000);
	mdelay(10);
	...
	writel(gpgcon_bkup, S3C2410_GPGCON);
	writel(gpgdat_bkup, S3C2410_GPGDAT);
	writel(gpgup_bkup, S3C2410_GPGUP);
...
static int __init s3c2410fb_probe(struct platform_device *pdev)
{
	...
	spi_lcd_init(); // eL2440 Board spi init..
	...
	s3c2410fb_set_par(fbinfo); //added
