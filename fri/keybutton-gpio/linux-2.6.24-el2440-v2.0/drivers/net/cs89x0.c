...
static unsigned char s3c2440_cs89x0_hw_addr[6] = {0x00, 0x00, 0xc0, 0xff, 0xee, 0x08};
#endif
...
#ifdef CONFIG_CPU_S3C2440
static unsigned int netcard_portlist[] __initdata = { (int)(0xF6000000+0x300), 0};
...
#ifdef CONFIG_CPU_S3C2440
	writel(( (readl(S3C2410_BWSCON) & ~(0xf000)) | (S3C2410_BWSCON_ST3 | S3C2410_BWSCON_WS3 | S3C2410_BWSCON_DW3_16) ), S3C2410_BWSCON);
	writel( ( S3C2410_BANKCON_Tacs0 | S3C2410_BANKCON_Tcos4 | S3C2410_BANKCON_Tacc14 | S3C2410_BANKCON_Tcoh1 | S3C2410_BANKCON_Tcah4 | S3C2410_BANKCON_Tacp6 | S3C2410_BANKCON_PMC16 ) , S3C2410_BANKCON3);

	writel(( (readl(S3C2410_GPGCON) & ~(0x3<<2)) | S3C2410_GPG1_EINT9 ), S3C2410_GPGCON);
	writel(( (readl(S3C2410_GPGUP) & ~(0x1<<1)) | (0x1<<1) ), S3C2410_GPGUP);
	writel(( (readl(S3C2410_EXTINT1) & ~(0x7<<4)) | (S3C2410_EXTINT_RISEEDGE<<4) ), S3C2410_EXTINT1);
#endif
...
/*	if (!request_region(ioaddr & ~3, NETCARD_IO_EXTENT, DRV_NAME)) {
	...
	if (!request_mem_region(ioaddr & ~3, NETCARD_IO_EXTENT, DRV_NAME)){
		printk(KERN_ERR "%s: request_region(0x%x, 0x%x) failed\n",
				DRV_NAME, ioaddr, NETCARD_IO_EXTENT);
		retval = -EBUSY;
		goto out1;
...
#ifdef CONFIG_CPU_S3C2440
	dev->dev_addr[0] = s3c2440_cs89x0_hw_addr[0];
	dev->dev_addr[1] = s3c2440_cs89x0_hw_addr[1];
	dev->dev_addr[2] = s3c2440_cs89x0_hw_addr[2];
	dev->dev_addr[3] = s3c2440_cs89x0_hw_addr[3];
	dev->dev_addr[4] = s3c2440_cs89x0_hw_addr[4];
	dev->dev_addr[5] = s3c2440_cs89x0_hw_addr[5];
#endif
...
{
#ifdef CONFIG_CPU_S3C2440
	if(chip_type  == CS8900)	writereg(dev, PP_CS8900_ISAINT, 0);
	else						writereg(dev, PP_CS8920_ISAINT, irq);
#else
	int i;
...
#endif
	{
#if !defined(CONFIG_MACH_IXDP2351) && !defined(CONFIG_ARCH_IXDP2X01) && !defined(CONFIG_ARCH_PNX010X)&& !defined(CONFIG_CPU_S3C2440)
